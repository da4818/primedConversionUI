...
#Initializes the serial connection to the Arduino board
    def __init__(self, serial_port='/dev/cu.usbmodem143201', baud_rate=9600,
        read_timeout=5):
...
#- I for INPUT - O for OUTPUT - P for INPUT_PULLUP MO13
...
#nternally sends b'WA{pin_number}:{analog_value}' over the serial connection
    def analog_write(self, pin_number, analog_value):
        command = (''.join(('WA', str(pin_number), ':',
            str(analog_value)))).encode()
        self.conn.write(command)

#To ensure we are properly closing our connection to the Arduino device
    def close(self):
        self.conn.close()
